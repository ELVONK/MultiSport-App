...
SECTION_LABELS = {
    ...
    "D": "=== SECTION D: LIVE / IN-PLAY GAMES ===",
}
...
def load_sectioned_csv(path: str) -> dict[str, pd.DataFrame]:
    ...
    if not p.exists():
        ...
        return {k: pd.DataFrame() for k in "ABCD"}

    raw_lines = p.read_text(encoding="utf-8").splitlines()

    sections: dict[str, list[str]] = {k: [] for k in "ABCD"}
    current = None
    ...
    for line in raw_lines:
        ...
        # Detect section start. The section-header row is written as the
        # label text followed by empty cells (e.g. "=== SECTION A: ... ===,,,,"),
        # so a plain prefix match against the full label text is what we need —
        # matching against a fragment of the label (as an earlier version of
        # this function did) never matches, since the line still starts with
        # the leading "===" characters.
        for label_text, key in label_map.items():
            if stripped.startswith(label_text):
                current = key
...
df_live    = dfs["D"]   # live / in-play (time isn't comparable, shown separately)
...
c4.metric("🟣 Live / In-Play",      len(df_live))
c5.metric("📋 Total Games Seen",    len(df_disc) + len(df_nodis) + len(df_unmatch) + len(df_live))
...
# ─────────────────────────────────────────────────────────────────────────────
tab_a, tab_b, tab_c, tab_d, tab_charts = st.tabs([
    "🔴 With Discrepancy",
    "🟢 No Discrepancy",
    "🟡 Unmatched",
    "🟣 Live / In-Play",
...
# ── Tab D: Live / In-Play ─────────────────────────────────────────────────────
with tab_d:
    st.subheader(f"Live / In-Play Games  ({len(df_live)} games)")
    st.caption(
        "Games where one or both sources show an in-play marker (e.g. '34\\'') "
        "instead of a scheduled kickoff time. These aren't scored as time "
        "discrepancies — the game has already started, so comparing a live "
        "minute to a scheduled clock time wouldn't be meaningful."
    )
    if df_live.empty:
        st.info("No live games matched between the two sources right now.")
    else:
